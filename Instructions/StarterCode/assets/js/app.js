...
d3.csv("data.csv", function(err, censusData) {
    ...
    var state = "State: "
    var poverty = "In Poverty(%): "
    var active = "Physically Active(%): "
    ...
    chart.selectAll("circle")
        ...
        .on("mouseover", function (data) {
            div.transition()
                .duration(100)
                .style("opacity", .9);
            div.html(state.bold() + data.state + "<br/>" + poverty.bold() + data.percBelowPov + "<text>%</text>" + "<br/>" + active.bold() + data.physActive + "<text>%</text>")
